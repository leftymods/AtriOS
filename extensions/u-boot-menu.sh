...
function extension_prepare_config__add_uboot_menu_pkg() {
	add_packages_to_image "u-boot-menu" # @TODO: using Debian/Ubuntu default package. AtriOS will need to provide its own, later, so we can customize.
}
...
function pre_umount_final_image__configure_uboot_menu() {
	...
	cat <<- UBOOT_MENU_CONFIGURATION_COMMON > "${MOUNT}/etc/default/u-boot"
		...
		# Generated by AtriOS for board ${BOARD} ${BRANCH}
		U_BOOT_UPDATE="true"
		U_BOOT_ALTERNATIVES="default recovery"
		U_BOOT_MENU_LABEL="AtriOS"
		U_BOOT_TIMEOUT="10"
