...
# Load atriosEnv.txt if it exists - using direct load instead of test -e for better compatibility
# This happens AFTER PARTUUID detection so user can override rootdev if needed
echo "Attempting to load ${prefix}atriosEnv.txt from ${devtype} ${devnum}:${distro_bootpart} to ${load_addr}..."
if load ${devtype} ${devnum}:${distro_bootpart} ${load_addr} ${prefix}atriosEnv.txt; then
	echo "Successfully loaded atriosEnv.txt (${filesize} bytes)"
	if env import -t ${load_addr} ${filesize}; then
		echo "Successfully imported environment from atriosEnv.txt"
	else
		echo "Warning: Failed to import environment from atriosEnv.txt"
	fi
else
	echo "atriosEnv.txt not found or failed to load - using default environment"
fi
