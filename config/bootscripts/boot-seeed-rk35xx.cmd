...
# Load atriosEnv.txt with corruption detection and .bak fallback.
...
if load ${devtype} ${devnum}:${distro_bootpart} ${load_addr} ${prefix}atriosEnv.txt; then
	if env import -t ${load_addr} ${filesize}; then
		...
		echo "WARNING: atriosEnv.txt format error"
	fi
fi
if test "${armbian_env_loaded}" = "no"; then
	if load ${devtype} ${devnum}:${distro_bootpart} ${load_addr} ${prefix}atriosEnv.txt.bak; then
		echo "Loading atriosEnv.txt.bak as fallback"
		if env import -t ${load_addr} ${filesize}; then
			setenv armbian_env_loaded "yes"
		else
			echo "ERROR: atriosEnv.txt.bak format error"
		fi
	else
		echo "WARNING: no atriosEnv.txt.bak found"
	fi
fi
...
# fdtfile is set by atriosEnv.txt (per-board BOOT_FDT_FILE).
