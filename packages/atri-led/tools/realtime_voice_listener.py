...
class RealtimeVoiceBiometrics:
    def __init__(self, base_dir=None):
        if not base_dir or not os.path.exists(os.path.join(base_dir, 'body.tflite')):
            candidates = [
        ...
        print(f"[+] Initializing Yandex ECAPA-TDNN neural networks from {base_dir}...")
                ...
                try:
                    ...
                    if rms > VAD_ENERGY_THRESHOLD and len(buffer) >= int(0.8 * SAMPLE_RATE):
                        ...
                        if emb is not None and len(self.enrolled_profiles) > 0:
                            ...
                            line = f"RMS: {rms:.3f} |{meter:<25}| {status_msg}"
                            print(line, flush=True)
                        else:
                            line = f"RMS: {rms:.3f} |{meter:<25}| [РЕЧЬ ОБНАРУЖЕНА]"
                            print(line, flush=True)
                    else:
                        if int(current_time * 2) % 4 == 0:  # Периодический отчет о фоне
                            print(f"RMS: {rms:.3f} |{meter:<25}| [ТИШИНА / ФОН]", flush=True)
