...
function kernel_prepare_bare_repo_decide_shallow_or_full() {
	...
	declare base_oras_ref="${GHCR_SOURCE}/leftymods/coreos" # @TODO allow changing this
		...
		if [[ -n "${KERNELSOURCE}" && "${KERNELSOURCE}" != "https://git.kernel.org/"* && "${KERNELSOURCE}" != "https://kernel.googlesource.com/"* ]]; then
			...
			if [[ -d "${kernel_git_bare_tree}/.git" ]]; then
				...
				if ! git -C "${kernel_git_bare_tree}" show-ref --verify --quiet refs/heads/main; then
					...
					run_host_command_logged git -C "${kernel_git_bare_tree}" fetch --depth=1 origin "${fetch_ref}:main"
