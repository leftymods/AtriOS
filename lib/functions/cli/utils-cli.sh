...
#	declare -A -g AtriOS_PARSED_CMDLINE_PARAMS=()
#	declare -a -g AtriOS_NON_PARAM_ARGS=()
...
function parse_cmdline_params() {
	...
	for arg in "${@}"; do
		if [[ "${arg}" == *=* ]]; then # contains an equal sign. it's a param.
			...
			AtriOS_NON_PARAM_ARGS+=("${non_param_value}")
		fi
	done
}
...
# This is called:
# apply_cmdline_params_to_env "reason" # reads from global AtriOS_PARSED_CMDLINE_PARAMS
function apply_cmdline_params_to_env() {
	declare -A -g AtriOS_PARSED_CMDLINE_PARAMS # Hopefully this has values
	declare __my_reason="${1}"
	shift

	# Loop over the dictionary and apply the values to the environment.
	for param_name in "${!AtriOS_PARSED_CMDLINE_PARAMS[@]}"; do
		...
		param_value="${AtriOS_PARSED_CMDLINE_PARAMS[${param_name}]}"
		param_value_desc="${param_value:-(empty)}"
...
function parse_each_cmdline_arg_as_command_param_or_config() {
	...
	if [[ "${is_command}" == "yes" && "${is_config}" == "yes" ]]; then
		exit_with_error "You cannot have a configuration file named '${config_file}'. '${argument}' is a command name and is reserved for internal AtriOS usage. Sorry. Please rename your config file and pass its name it an argument, and I'll use it. PS: You don't need a config file for 'docker' anymore, Docker is all managed by AtriOS now."
...
function cli_standard_relaunch_docker_or_sudo() {
	...
	if [[ "${EUID}" == "0" ]]; then # we're already root. Either running as real root, or already sudo'ed.
		if [[ "${AtriOS_RELAUNCHED}" != "yes" && "${ALLOW_ROOT}" != "yes" ]]; then
