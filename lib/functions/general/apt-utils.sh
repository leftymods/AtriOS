...
function apt_find_upstream_package_version_and_download_url() {
	...
	# Try JSON metadata first (Armbian-style generated data)
	declare package_info_download_url=""
	...
	found_package_filename=$(
		jq -r --arg release "${package_download_release}" --arg arch "${ARCH}" \
			'.[$release][$arch]' "$package_info_download_url_file" 2>/dev/null || true
	)
	...
	# Fallback: if JSON failed, query apt cache directly
	if [[ "${found_package_filename}" != "${sought_package_name}_"* ]]; then
		display_alert "JSON metadata not available, falling back to apt-cache" "${sought_package_name}" "info"
		declare apt_version
		apt_version="$(apt-cache show "${sought_package_name}" 2>/dev/null | grep -E '^Version:' | head -1 | awk '{print $2}')"
		if [[ -n "${apt_version}" ]]; then
			found_package_filename="${sought_package_name}_${apt_version}_${ARCH}.deb"
			# Debian/Ubuntu versions can contain epoch (e.g. 1:13.2+ds-2), strip epoch for URL
			found_package_version="${apt_version}"
			found_package_down_url="${base_down_url}/${found_package_filename}"
			display_alert "Found package via apt-cache" "${found_package_filename}" "info"
