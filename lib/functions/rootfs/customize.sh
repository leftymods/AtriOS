...
function customize_image() {
	...
	call_extension_method "pre_customize_image" "image_tweaks_pre_customize" <<- 'PRE_CUSTOMIZE_IMAGE'
		*run before customize-image.sh*
		...
		Attention: only the Distro default repos are enabled at this point; no packages from AtriOS or custom repos can be used.
		If you need repos, please consider `post_AtriOS_repo_customize_image` or `post_repo_customize_image`.
	PRE_CUSTOMIZE_IMAGE
	...
	call_extension_method "post_customize_image" "image_tweaks_post_customize" <<- 'POST_CUSTOMIZE_IMAGE'
		*post customize-image.sh hook*
		...
		Attention: only the Distro default repos are enabled at this point; no AtriOS or custom repos can be used.
...
function run_hooks_post_AtriOS_repo_customize_image() {
	call_extension_method "post_AtriOS_repo_customize_image" <<- 'post_AtriOS_repo_customize_image'
		*run after post_customize_image, after and only if AtriOS standard repos have been enabled*
		All repos have been enabled, including the AtriOS repo and custom ones.
		You can install packages from the AtriOS repo here.
	post_AtriOS_repo_customize_image
	return 0
}
...
function run_hooks_post_repo_customize_image() {
	call_extension_method "post_repo_customize_image" <<- 'post_repo_customize_image'
		*run after post_customize_image, after repos have been enabled*
		All repos have been enabled, including custom ones; AtriOS repo is not guaranteed to be enabled.
		You can install packages from the default Debian/Ubuntu repos, or custom repos, here.
		To install packages from the AtriOS repo, use the post_AtriOS_repo_customize_image hook.
