...
function create_new_rootfs_cache_via_debootstrap() {
	...
	echo 'APT::Sandbox::User "root";' > "${SDCARD}/etc/apt/apt.conf.d/99-atrios-sandbox"
	...
	rm -f "${SDCARD}/etc/apt/apt.conf.d/99-atrios-sandbox" # build-time only; don't ship in the image
	...
	# for the AtriOS repo, only the components which are _not_ produced by AtriOS/build are included (-desktop and -utils)
	...
	display_alert "Upgrading base packages" "AtriOS" "info"
	do_with_retries 3 chroot_sdcard_apt_get upgrade

	# stage: install additional packages
	display_alert "Installing the main packages for" "AtriOS" "info"
	declare -g if_error_detail_message="Installation of AtriOS main packages for ${RELEASE} ${DESKTOP_ENVIRONMENT:-cli} ${DESKTOP_TIER:-none} ${BUILD_MINIMAL} failed"
	...
	# display_alert "Purging residual packages for" "AtriOS" "info"
	...
	# `AtriOS-first-run` will do the same thing later
