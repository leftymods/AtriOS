...
### Процедура поиска реальных пинов ручки

Сток декларирует A=GPIOA_0, B=AO10, но AO10 занят регулятором 20V.
Прежде чем мириться с отключенной ручкой — измерь фактические пины:

```bash
atri-hwprobe --watch-gpio 30
# и МЕДЛЕННО крути ручку все 30 секунд
```

- Занятые ядром линии будут перечислены с владельцем (unwatchable)
- Крутящиеся при повороте линии = фазы энкодера

Интерпретация:
1. **Две свободные линии дёргаются** (например `periphs-banks line 49`
   и что-то ещё) → это A/B. Правка: `ROTARY_GPIO_B="aobus-banks N"`
   (или periphs-banks) в `config/boards/atristation.conf` → образ →
   ручка живая, PVDD остаётся на AO10. Конфликта больше нет!
2. **Дёргается только GPIOA_0** (49) → фаза B реально сидит на
   занятой AO10 (regulator 20V_AMPL будет указан в claimed-списке).
   Тогда: либо ручка остаётся отключенной, либо эксперимент с
   переносом PVDD обратно на X10 + проверка звука мультиметром.
3. Ни одна не дёргается → проверить питание лазерного модуля
   (`atri-hwprobe | grep -A5 "GPIO chips"` — consumer у линий A/B).
