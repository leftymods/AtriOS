...
✅ Пин PVDD сверён с оригинальным DTB: **GPIOAO_10** (`gpio_ampl_pwr`,
initial-low, vendor поднимал после init DSP). Наш регулятор 20V_AMPL
привязан к нему; anti-pop секвенция драйвера подаёт его шагом 4.

**Цена**: лазерный энкодер, фаза B тоже AO10 (сток делил этот пин).
С регулятором на линии `rotary-poll` не может её экспортировать →
при старте пишет явную ошибку «volume knob DISABLED» и завершается.
Ручка громкости отключена до реверса этой платы/решения; фаза A
(GPIOA_0) остаётся свободной.

Если когда-нибудь подтвердится альтернативная ревизия с питанием на
GPIOX_10: вернуть gpio регулятора на X10 — энкодер оживёт полностью
(обе фазы свободны).
...
⚠ Фаза B (AO10) занята регулятором 20V_AMPL (совпадение со стоком,
где этот же пин был ampl_pwr) → `rotary-poll` детектирует это при
старте и завершается с ошибкой «volume knob DISABLED». Ручка
неактивна в текущей конфигурации; варианты решения — см. раздел
anti-pop выше.
